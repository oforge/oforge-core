...
class PagesControllerService extends AbstractDatabaseAccess {
    ...
    public function editPageData($post) {
        ...
        switch ($selectedAction) {
            ...
            case 'rename':
                ...
                break;
            case 'duplicate':
                if (isset($post['cms_edit_page_id'])) {
                    /** @var PageDuplicateService $pageDuplicateService */
                    $pageDuplicateService = Oforge()->Services()->get('page.duplicate');

                    $tmpPageData = $pageDuplicateService->duplicate($post['cms_edit_page_id']);
                    if ($tmpPageData !== false) {
                        $selectedPageId   = $tmpPageData['pageID'];
                        $selectedPageName = $tmpPageData['pageName'];
                    }
                }
                break;
            case 'delete':
    ...
    public function checkForValidPagePath($post) {
        $selectedPage     = isset($post["cms_page_jstree_selected_page"])
                            && $post["cms_page_jstree_selected_page"] > 0 ? $post["cms_page_jstree_selected_page"] : 0;
        $selectedLanguage = isset($post["cms_page_selected_language"])
                            && $post["cms_page_selected_language"]
                               > 0 ? $post["cms_page_selected_language"] : $post["cms_page_selected_language"] = $this->getDefaultLanguageForPage($selectedPage);
    ...
    public function deleteContentElement($pagePathId, $selectedElementId, $deleteContentWithId, $deleteContentAtOrderIndex) {
        ...
        if ($contentElementId && $contentElementAtOrderIndex) {
            if ($containerElementId) {
                ...
                if ($pagePathId !== false) {
                    $pageContentEntity = $this->repository('pageContent')->findOneBy([
                        "pagePath" => $pagePathId,
    ...
    public function editContentData($post) {
        ...
        $selectedLanguage          = isset($post["cms_page_selected_language"])
                                     && $post["cms_page_selected_language"]
                                        > 0 ? $post["cms_page_selected_language"] : $post["cms_page_selected_language"] = $this->getDefaultLanguageForPage($selectedPage);
        $selectedElement           = isset($post["cms_page_selected_element"])
                                     && !empty($post["cms_page_selected_element"]) ? $post["cms_page_selected_element"] : 0;
        $createContentWithTypeId   = isset($post["cms_page_create_content_with_type_id"])
                                     && $post["cms_page_create_content_with_type_id"] > 0 ? $post["cms_page_create_content_with_type_id"] : 0;
        $createContentAtOrderIndex = isset($post["cms_page_create_content_at_order_index"])
                                     && $post["cms_page_create_content_at_order_index"] > 0 ? $post["cms_page_create_content_at_order_index"] : 0;
        $deleteContentWithId       = isset($post["cms_page_delete_content_with_id"])
                                     && !empty($post["cms_page_delete_content_with_id"]) ? $post["cms_page_delete_content_with_id"] : 0;
        $deleteContentAtOrderIndex = isset($post["cms_page_delete_content_at_order_index"])
                                     && !empty($post["cms_page_delete_content_at_order_index"]) ? $post["cms_page_delete_content_at_order_index"] : 0;
        $selectedAction            = isset($post["cms_page_selected_action"])
                                     && !empty($post["cms_page_selected_action"]) ? $post["cms_page_selected_action"] : 'edit';
        ...
        if ($selectedPage) {
            ...
            if ($selectedElement) {
                ...
                if (is_numeric($selectedElementId) && $selectedElementId > 0) {
                    ...
                    switch ($selectedAction) {
                        case "create":
                            $newContentId = $this->createContentElement($pageArray["paths"][$selectedLanguage]["id"], $selectedElementId,
                                $createContentWithTypeId, $createContentAtOrderIndex);
                            if ($newContentId) {
                            ...
                            break;
                        case "delete":
                            $this->deleteContentElement($pageArray["paths"][$selectedLanguage]["id"], $selectedElementId, $deleteContentWithId,
                                $deleteContentAtOrderIndex);
                        ...
                        case "submit":
                            ...
                            $data["contentElementData"] = $contentTypeService->setContentDataArray($selectedElementId, $selectedElementTypeId, $post)
                                                                             ->getContentDataArray($selectedElementId, $selectedElementTypeId);
                            $data["contents"]           = $pageBuilderService->getContentDataArrayById($pageContents, $selectedElement);
                            break;
                        default:
                ...
                switch ($selectedAction) {
                    case "create":
                        $newContentId = $this->createContentElement($pageArray["paths"][$selectedLanguage]["id"], $selectedElementId, $createContentWithTypeId,
                            $createContentAtOrderIndex);
                        if ($newContentId) {
