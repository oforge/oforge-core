...
class DoctrineHelper
{
     ...
     * @return array
     */
    public static function toArray(array $entities, ?string $keyMethod = null, int $maxDepth = 2, array $excludeProperties = []) : array
    {
        $result = [];
        foreach ($entities as $index => $entity) {
            $key = ($keyMethod !== null && method_exists($entity, $keyMethod) ? $entity->$keyMethod() : $index);

            $result[$key] = $entity->toArray($maxDepth, $excludeProperties);
        }
